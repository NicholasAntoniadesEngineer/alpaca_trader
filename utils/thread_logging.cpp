...
void ThreadLogging::log_startup_configuration(const TimingConfig& config) {
    ...
    log_message("   |  • Main Thread: " + ThreadSystem::Platform::ThreadControl::get_thread_info(), "trade_log.txt");
}
...
void ThreadLogging::log_priority_result(const std::string& thread_name, const std::string& priority_tag, 
                                       const std::string& requested_priority, const std::string& actual_priority, 
                                       bool success, bool has_affinity, int cpu_affinity) {
    ...
    log_message(create_priority_status(thread_name, priority_info, success, priority_tag, affinity_info), "trade_log.txt");
}
...
void ThreadLogging::log_logger_priority_result(const std::string& requested_priority, const std::string& actual_priority, bool success) {
    std::string priority_info = (requested_priority != actual_priority && success) ? 
        requested_priority + " (scaled to " + actual_priority + ")" : actual_priority;
    log_message(create_priority_status("LOGGER", priority_info, success, "[LOWEST]"), "trade_log.txt");
}
...
void ThreadLogging::log_performance_summary(const SystemThreads& handles) {
    ...
    if (elapsed_sec > 0) {
        ...
        log_message("      └─ [LOWEST]   LOGGER: " + format_number(handles.logger_iterations.load()) + 
                   " iterations (" + std::to_string(handles.logger_iterations.load() / elapsed_sec) + "/sec)", "trade_log.txt");
        log_message("", "trade_log.txt");
    }
}
