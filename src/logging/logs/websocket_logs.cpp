...
void AlpacaTrader::Logging::WebSocketLogs::log_websocket_bar_data_table(const std::string& symbolString, double openPriceValue, double highPriceValue, double lowPriceValue, double closePriceValue, double volumeValue, const std::string& timestampString, const std::string& logFileString) {
    std::string headerLineOneValue = "┌───────────────────┬──────────────────────────────────────────────────┐";
    log_message(headerLineOneValue, logFileString);
    std::string headerLineTwoValue = "│ XAS Bar Data     │ WebSocket Aggregate Per Second Bar                 │";
    ...
    std::string symbolRowValue = symbolString;
    ...
    symbolRowLineValue += " │";
    log_message(symbolRowLineValue, logFileString);
    
    std::ostringstream priceStream;
    priceStream << std::fixed << std::setprecision(2);
    priceStream << "O: " << openPriceValue << " H: " << highPriceValue << " L: " << lowPriceValue << " C: " << closePriceValue;
    std::string priceRowValue = priceStream.str();
    std::string priceRowLineValue = "│ OHLC              │ " + priceRowValue.substr(0, 48);
    if (priceRowValue.length() < 48) {
        priceRowLineValue += std::string(48 - priceRowValue.length(), ' ');
    }
    priceRowLineValue += " │";
    log_message(priceRowLineValue, logFileString);
    
    std::string volumeRowValue = "Volume: " + std::to_string(static_cast<long long>(volumeValue));
    std::string volumeRowLineValue = "│ Volume            │ " + volumeRowValue.substr(0, 48);
    if (volumeRowValue.length() < 48) {
        volumeRowLineValue += std::string(48 - volumeRowValue.length(), ' ');
    }
    volumeRowLineValue += " │";
    log_message(volumeRowLineValue, logFileString);
    ...
    std::string timestampRowValue = "Timestamp: " + formattedTimestampString;
...
void AlpacaTrader::Logging::WebSocketLogs::log_websocket_accumulator_status(const std::string& symbolString, size_t barsBeforeValue, size_t barsAfterValue, size_t firstLevelCountValue, size_t secondLevelCountValue, const std::string& logFileString) {
    std::string headerLineOneValue = "┌───────────────────┬──────────────────────────────────────────────────┐";
    log_message(headerLineOneValue, logFileString);
    std::string headerLineTwoValue = "│ Accumulator Status│ Bar Accumulator State Information               │";
    log_message(headerLineTwoValue, logFileString);
    std::string headerLineThreeValue = "├───────────────────┼──────────────────────────────────────────────────┤";
    log_message(headerLineThreeValue, logFileString);
    ...
    std::string barsBeforeRowValue = "Bars before: " + std::to_string(barsBeforeValue);
    std::string barsBeforeRowLineValue = "│ Before            │ " + barsBeforeRowValue.substr(0, 48);
    if (barsBeforeRowValue.length() < 48) {
        barsBeforeRowLineValue += std::string(48 - barsBeforeRowValue.length(), ' ');
    }
    barsBeforeRowLineValue += " │";
    log_message(barsBeforeRowLineValue, logFileString);
    
    std::string barsAfterRowValue = "Bars after: " + std::to_string(barsAfterValue);
    std::string barsAfterRowLineValue = "│ After             │ " + barsAfterRowValue.substr(0, 48);
    if (barsAfterRowValue.length() < 48) {
        barsAfterRowLineValue += std::string(48 - barsAfterRowValue.length(), ' ');
    }
    barsAfterRowLineValue += " │";
    log_message(barsAfterRowLineValue, logFileString);
    
    std::string levelsRowValue = "1st Level: " + std::to_string(firstLevelCountValue) + " | 2nd Level: " + std::to_string(secondLevelCountValue);
    std::string levelsRowLineValue = "│ Levels            │ " + levelsRowValue.substr(0, 48);
    if (levelsRowValue.length() < 48) {
        levelsRowLineValue += std::string(48 - levelsRowValue.length(), ' ');
    }
    levelsRowLineValue += " │";
    log_message(levelsRowLineValue, logFileString);
...
void AlpacaTrader::Logging::WebSocketLogs::log_websocket_bars_request_table(const std::string& symbolString, int requestedBarsCountValue, size_t availableBarsCountValue, size_t firstLevelBarsCountValue, size_t secondLevelBarsCountValue, const std::string& logFileString) {
    std::string headerLineOneValue = "┌───────────────────┬──────────────────────────────────────────────────┐";
    log_message(headerLineOneValue, logFileString);
    std::string headerLineTwoValue = "│ Bars Request      │ WebSocket Bar Request Details                     │";
    log_message(headerLineTwoValue, logFileString);
    std::string headerLineThreeValue = "├───────────────────┼──────────────────────────────────────────────────┤";
    log_message(headerLineThreeValue, logFileString);
    
    std::string symbolRowLineValue = "│ Symbol            │ " + symbolString.substr(0, 48);
    if (symbolString.length() < 48) {
        symbolRowLineValue += std::string(48 - symbolString.length(), ' ');
    }
    symbolRowLineValue += " │";
    log_message(symbolRowLineValue, logFileString);
    
    std::string requestedRowValue = "Requested: " + std::to_string(requestedBarsCountValue) + " bars";
    std::string requestedRowLineValue = "│ Requested         │ " + requestedRowValue.substr(0, 48);
    if (requestedRowValue.length() < 48) {
        requestedRowLineValue += std::string(48 - requestedRowValue.length(), ' ');
    }
    requestedRowLineValue += " │";
    log_message(requestedRowLineValue, logFileString);
    
    std::string availableRowValue = "Available: " + std::to_string(availableBarsCountValue) + " bars";
    std::string availableRowLineValue = "│ Available         │ " + availableRowValue.substr(0, 48);
    if (availableRowValue.length() < 48) {
        availableRowLineValue += std::string(48 - availableRowValue.length(), ' ');
    }
    availableRowLineValue += " │";
    log_message(availableRowLineValue, logFileString);
    
    std::string levelsRowValue = "1st Level: " + std::to_string(firstLevelBarsCountValue) + " | 2nd Level: " + std::to_string(secondLevelBarsCountValue);
    std::string levelsRowLineValue = "│ Levels            │ " + levelsRowValue.substr(0, 48);
    if (levelsRowValue.length() < 48) {
        levelsRowLineValue += std::string(48 - levelsRowValue.length(), ' ');
    }
